import{_ as e,o as a,c as n,a as r}from"./app-3d979d38.js";const t={},c=r(`<h1 id="strace-跟踪进程中的系统调用" tabindex="-1"><a class="header-anchor" href="#strace-跟踪进程中的系统调用" aria-hidden="true">#</a> strace 跟踪进程中的系统调用</h1><p>strace常用来跟踪进程执行时的系统调用和所接收的信号。 在Linux世界，进程不能直接访问硬件设备，当进程需要访问硬件设备(比如读取磁盘文件，接收网络数据等等)时，必须由用户态模式切换至内核态模式，通过系统调用访问硬件设备。strace可以跟踪到一个进程产生的系统调用,包括参数，返回值，执行消耗的时间。</p><h2 id="输出参数含义" tabindex="-1"><a class="header-anchor" href="#输出参数含义" aria-hidden="true">#</a> 输出参数含义</h2><p>每一行都是一条系统调用，等号左边是系统调用的函数名及其参数，右边是该调用的返回值。 strace 显示这些调用的参数并返回符号形式的值。strace 从内核接收信息，而且不需要以任何特殊的方式来构建内核。</p><pre><code>$strace cat /dev/null 
...
</code></pre><p>跟踪28979进程的所有系统调用（-e trace=all），并统计系统调用的花费时间，以及开始时间（并以可视化的时分秒格式显示），最后将记录结果存在output.txt文件里面。</p>`,14),s=[c];function o(i,d){return a(),n("div",null,s)}const h=e(t,[["render",o],["__file","strace.html.vue"]]);export{h as default};
